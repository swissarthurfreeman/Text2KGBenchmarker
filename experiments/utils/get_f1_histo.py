...
for i in [1, 2, 3, 4, 5, 6]:
    for model_name in [f'gpt-4o-{i}-shot', f'gpt-3.5-turbo-{i}-shot']:
        ...
        with open("../results/metrics/" + model_name + "/dpedia_webnlg_clean_avg.jsonl") as f:
...
ax.grid(True)
plt.title("#occurences, all shots, GPT-3.5/4o")
plt.xlabel("F1 Score\nWikidata-TekGen All")
plt.hist(wikidata_all_f1s_gpt35and4oAllShots, bins=15)
...
plt.title("#occurences, all shots, GPT-3.5/4o")
plt.xlabel("F1 Score\nDBpedia-WebNLG")
plt.hist(dbpedia_f1s_gpt35and4oAllShots, bins=15)
plt.show()
